...
const TransformationShowcase: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('open-transformation-showcase', handler as EventListener);
    return () => window.removeEventListener('open-transformation-showcase', handler as EventListener);
  }, []);
    ...
    <AnimatePresence>
      {open && (
        <motion.div className="fixed inset-0 z-[100] flex items-center justify-center p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          ...
          <motion.div className="relative w-full max-w-5xl bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden" initial={{ y: 40, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 20, opacity: 0 }} transition={{ duration: 0.25 }}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-gray-50">
              <h3 className="text-lg sm:text-xl font-bold text-gray-900">How Alumni Nexus Transforms Engagement</h3>
              <button aria-label="Close" onClick={() => setOpen(false)} className="p-2 rounded-lg hover:bg-gray-100"><X className="w-5 h-5" /></button>
            </div>
            <div className="p-6 sm:p-8">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                <motion.div
                  ...
                  whileHover={{ y: -4, scale: 1.02, boxShadow: '0 16px 32px rgba(0,0,0,0.06)' }}
                >
                ...
                >
                  ...
                  <p className="text-sm text-gray-600">AI-assisted mentor/mentee matching.</p>
                </motion.div>
                <motion.div
                ...
                >
                  ...
                  <p className="text-sm text-gray-600">Integrated payments and receipts.</p>
                </motion.div>
                <motion.div
                ...
                >
                  ...
                  <p className="text-sm text-gray-600">Plan events, RSVPs, and follow-ups.</p>
                </motion.div>
              </div>
              <div className="mt-6 flex justify-end"><button className="px-4 py-2 rounded-lg border border-gray-200 text-gray-700 hover:bg-gray-50" onClick={() => setOpen(false)}>Close</button></div>
            </div>
          </motion.div>
        </motion.div>
      )}
